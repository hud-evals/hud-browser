...
## Option B: CLI Evaluation
    ...
    # Run all tasks
    hud eval my-org/browser-tasks --model gpt-4o --remote

## Option C: Python Script (this file)

    # Upload tasks to platform
    python remote_test.py --upload my-org/browser-tasks

    # Run all tasks
    python remote_test.py --platform my-org/browser-tasks --model gpt-4o
...
async def main():
    ...
    parser.add_argument(
        "--upload",
        metavar="SLUG",
        help="Upload tasks to platform (e.g. my-org/browser-tasks)",
    )
    parser.add_argument(
        "--platform",
        metavar="SLUG",
